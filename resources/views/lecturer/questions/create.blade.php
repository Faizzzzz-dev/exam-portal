<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('lecturer.questions.index', $exam) }}" class="text-gray-600 hover:text-gray-900">
                ← Back to Questions
            </a>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add New Question</h2>
        </div>
    </x-slot>
                    ...
                    <form action="{{ route('lecturer.questions.store', $exam) }}" method="POST" id="questionForm">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type" required onchange="toggleQuestionType()"
                                ...
                                <option value="open_text" {{ old('type') == 'open_text' ? 'selected' : '' }}>
                                    Open Text
                                </option>
                            </select>
                            @error('type')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="question_text" id="question_text" rows="4" required
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 @error('question_text') border-red-500 @enderror">{{ old('question_text') }}</textarea>
                            @error('question_text')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="marks" id="marks" value="{{ old('marks', 1) }}" required min="1"
                            ...
                            <div class="mb-4">
                                ...
                                <div id="optionsContainer" class="space-y-3">
                                    <div class="option-item flex items-center space-x-3">
                                        <span class="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center text-sm font-medium">A</span>
                                        <input type="text" name="options[0][text]" placeholder="Option A" required
                                    ...
                                    <div class="option-item flex items-center space-x-3">
                                        <span class="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center text-sm font-medium">B</span>
                                        <input type="text" name="options[1][text]" placeholder="Option B" required
                                            class="flex-1 shadow appearance-none border rounded py-2 px-3 text-gray-700">
                        ...
                        <div class="flex items-center justify-between">
                            <a href="{{ route('lecturer.questions.index', $exam) }}" class="bg-gray-500 hover:bg-gray-700 text-black font-bold py-2 px-4 rounded">
                                Cancel
                            </a>
    ...
    <script>
        ...
        function addOption() {
            ...
            optionDiv.innerHTML = `
                <span class="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center text-sm font-medium">${letter}</span>
                <input type="text" name="options[${optionCount}][text]" placeholder="Option ${letter}" required
                    class="flex-1 shadow appearance-none border rounded py-2 px-3 text-gray-700">
